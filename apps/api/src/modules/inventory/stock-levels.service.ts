...
import { paginationParams, qty, serializeStockLevel } from "./mappers.js";
import { computeStockValuation } from "./valuation.service.js";
  ...
  const lowStock = levels
    ...
    .map((level) => ({
      ...
      product: {
        ...
        lowStockThreshold: qty(level.product.lowStockThreshold),
      },
    }));
 ...
 *
 * The aggregation itself lives in `valuation.service.ts` (shared with
 * `GET /reports/inventory/valuation`, see that module's doc comment) —
 * this handler only maps the shared result onto this endpoint's existing
 * response shape and rounds at the response boundary.
 */
export async function getValuation(req: Request, res: Response) {
  const { tenant } = assertTenant(req);
  const warehouseId = typeof req.query.warehouseId === "string" ? req.query.warehouseId : undefined;

  const { totalValue, byWarehouse } = await computeStockValuation({
    businessId: tenant.businessId,
    warehouseId,
  });
