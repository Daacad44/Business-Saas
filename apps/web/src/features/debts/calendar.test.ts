...
describe("classifyDebtCalendar matches API overdueDebtWhere / dueTodayDebtWhere", () => {
  it("does not offer DUE_SOON as a status filter", () => {
    expect(DEBT_STATUS_FILTERS).not.toContain("DUE_SOON");
    expect([...DEBT_STATUS_FILTERS]).toEqual([
      "PENDING",
      "PARTIALLY_PAID",
      "PAID",
      "CANCELLED",
  ...
  it("labels a debt due at the start and end of the business calendar day as DUE_TODAY", () => {
    ...
    expect(classifyDebtCalendar(debt({ dueDate: todayStart }), asOf, TZ)).toBe("DUE_TODAY");
    expect(classifyDebtCalendar(debt({ dueDate: endOfToday }), asOf, TZ)).toBe("DUE_TODAY");
  });
  ...
  it("flips from DUE_TODAY to OVERDUE exactly at the next business midnight", () => {
    ...
    expect(classifyDebtCalendar(debt({ dueDate: lastSecondToday }), asOfToday, TZ)).toBe("DUE_TODAY");
    expect(classifyDebtCalendar(debt({ dueDate: lastSecondToday }), tomorrowStart, TZ)).toBe("OVERDUE");
    expect(classifyDebtCalendar(debt({ dueDate: tomorrowStart }), tomorrowStart, TZ)).toBe("DUE_TODAY");
  });
  ...
  it("treats a dueDate before the start of today as OVERDUE", () => {
    ...
    expect(classifyDebtCalendar(debt({ dueDate: yesterdayEnd }), asOf, TZ)).toBe("OVERDUE");
    expect(calendarDaysBetweenInTimezone(yesterdayEnd, asOf, TZ)).toBeGreaterThan(0);
  });
  ...
  it("does not label future-due open debts", () => {
    ...
    expect(classifyDebtCalendar(debt({ dueDate: tomorrow }), asOf, TZ)).toBeNull();
  });
  ...
  it("ignores PAID / CANCELLED / zero-outstanding rows even when dueDate is in the past", () => {
    ...
    expect(classifyDebtCalendar(debt({ dueDate: past, status: "PAID" }), asOf, TZ)).toBeNull();
    expect(classifyDebtCalendar(debt({ dueDate: past, status: "CANCELLED" }), asOf, TZ)).toBeNull();
    expect(classifyDebtCalendar(debt({ dueDate: past, outstandingAmount: "0.00" }), asOf, TZ)).toBeNull();
    expect(
  ...
  it("does not treat a stored OVERDUE enum as overdue when dueDate is still in the future", () => {
    ...
    expect(classifyDebtCalendar(debt({ dueDate: nextWeek, status: "OVERDUE" }), asOf, TZ)).toBeNull();
  });
});
